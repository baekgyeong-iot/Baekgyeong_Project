...
처리 결과:
- `game_led`가 `LEFT`, `RIGHT`, `OFF` 중 하나로 변경
- `/api/led` 응답과 MQTT `baekgyeong/led/control` payload에 `game_led` 포함
- MQTT LED 제어 이벤트명은 각각 `LED_LEFT`, `LED_RIGHT`, `LED_OFF`
...
| LED 제어 수신 | `baekgyeong/led/control` | `LED_UPDATE`, `LED_LEFT`, `LED_RIGHT`, `LED_OFF` |
